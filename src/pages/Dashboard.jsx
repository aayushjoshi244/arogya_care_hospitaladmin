...
const Dashboard = () => {
  ...
  // Onboarding setup view for unapproved hospitals/labs
  if (!isApproved) {
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        {hospitalStatus === 'SUSPENDED' ? (
          <div className="bg-red-50 border border-red-200/50 p-6 rounded-3xl text-red-800 flex items-start gap-4 shadow-sm shadow-red-100/30">
            <div className="p-3 bg-red-100 text-red-700 rounded-2xl shrink-0 mt-0.5 animate-pulse">
              <ShieldAlert size={24} />
            </div>
            <div className="space-y-1">
              <h3 className="font-extrabold text-sm tracking-tight text-slate-800">
                {isLab ? 'Diagnostic Lab Operations Suspended' : 'Hospital Operations Suspended'}
              </h3>
              <p className="text-xs text-slate-500 font-medium leading-relaxed">
                ...
                Facility operations, staff logins, and report upload actions are temporarily disabled until the suspension period completes or is lifted by the administration.
              </p>
            </div>
          </div>
        ) : (
          <div className="bg-amber-50 border border-amber-200/50 p-6 rounded-3xl text-amber-800 flex items-start gap-4 shadow-sm shadow-amber-100/30">
            ...
            <div className="space-y-1">
              <h3 className="font-extrabold text-sm tracking-tight text-slate-800">
                {isLab ? 'Diagnostic Lab Verification Awaiting Superadmin Review' : 'Hospital Verification Awaiting Superadmin Review'}
              </h3>
              <p className="text-xs text-slate-500 font-medium leading-relaxed">
                Your facility registration is currently undergoing verification by Arogya Care Super Admins. During this phase, you can start onboarding your staff, testing directory, and basic configuration settings.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          <div className={`grid grid-cols-1 ${isLab ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-5`}>
            
            {isLab ? (
              /* Step 1 for Lab: Onboard Lab Technicians */
              <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm shadow-slate-100/50 flex flex-col justify-between h-48 hover-scale">
                <div className="space-y-2">
                  ...
                  <h4 className="text-sm font-bold text-slate-800">1. Onboard Lab Technicians</h4>
                  <p className="text-[11px] text-slate-400 font-medium leading-relaxed">Add technical staff profiles, contact emails, and access control settings.</p>
                </div>
                <button 
              ...
              /* Step 1 for Hospital: Onboard Doctors */
              <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm shadow-slate-100/50 flex flex-col justify-between h-48 hover-scale">
                <div className="space-y-2">
                  <div className="w-10 h-10 bg-primary/10 text-primary rounded-xl flex items-center justify-center">
                    <Stethoscope size={20} />
            ...
            <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm shadow-slate-100/50 flex flex-col justify-between h-48 hover-scale">
              <div className="space-y-2">
                ...
                <h4 className="text-sm font-bold text-slate-800">
                  {isLab ? '2. Configure Lab Tests & Prices' : '2. Configure Lab Tests'}
                </h4>
            ...
            {/* Step 3: Add Medicines Stock (Hospitals only) */}
            {!isLab && (
              <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm shadow-slate-100/50 flex flex-col justify-between h-48 hover-scale">
                <div className="space-y-2">
        ...
        <div className="bg-slate-50 border border-slate-200/60 p-6 rounded-3xl space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-semibold text-slate-400">
            ...
            <div className="flex items-center gap-2 p-3 bg-white rounded-2xl border border-slate-100">
              <Lock size={14} className="text-slate-300" />
              <span>{isLab ? 'Lab Settings' : 'Hospital Settings'}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
          ...
          <>
            ...
            <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm shadow-slate-100/50 flex items-center justify-between hover-scale">
              <div className="space-y-1">
                ...
                <h3 className="text-2xl font-extrabold text-slate-800">{stats?.totalReports || 0}</h3>
              </div>
            ...
            <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm shadow-slate-100/50 flex items-center justify-between hover-scale">
              <div className="space-y-1">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Awaiting Review</p>
                <h3 className="text-2xl font-extrabold text-slate-800">{stats?.pendingReports || 0}</h3>
              </div>
            ...
            <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm shadow-slate-100/50 flex items-center justify-between hover-scale">
              <div className="space-y-1">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Lab Technicians</p>
                <h3 className="text-2xl font-extrabold text-slate-800">{stats?.totalTechnicians || 0}</h3>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-100 p-6 rounded-3xl shadow-sm shadow-slate-100/50 flex flex-col justify-between min-h-[400px]">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium">Monthly timeline of captured transaction payment fees</p>
            </div>
        ...
        <div className="space-y-6">
                ...
                <>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-xs font-bold text-slate-700">Manage Lab Technicians</p>
                        <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Enroll technicians and access control</p>
                      </div>
                    </div>
                    <ArrowRight size={16} className="text-slate-400 group-hover:translate-x-1 transition-transform" />
                  </button>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-xs font-bold text-slate-700">Configure Testing Directory</p>
                        <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Manage diagnostic tests and bill pricing</p>
                      </div>
                    </div>
                    <ArrowRight size={16} className="text-slate-400 group-hover:translate-x-1 transition-transform" />
                  </button>
          ...
          {/* Pending Alerts widget (Hospitals only) */}
          {!isLab && stats?.pendingDoctors > 0 && (
